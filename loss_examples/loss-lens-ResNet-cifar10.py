...
parser.add_argument('--bn', action='store_false', help='if used, batch normalization is disabled')
parser.add_argument('--res', action='store_false', help='if used, residule connection is disabled')
...
"""
example:
python loss-lens-ResNet-cifar10.py 
--model '/global/cfs/cdirs/m636/geniesse/ResNet20_models/resnet20_batch_norm_True_residual_False_seed_0_net.pkl' 
--data '/global/cfs/cdirs/m636/geshi/data' --output '/global/cfs/cdirs/m636/geniesse/ResNet20_loss_lens' --batch 512 
--seed 0 --device 0 --res --distance 0.05 --steps 40 --norm layer --random normal
"""
...
model = resnet(num_classes=10,
               depth=20,
               residual_not=use_res,
               batch_norm_not=use_bn)
...
output_name = os.path.join(output_dir, f"{model_name.split('/')[-1].split('.')[0]}_hessian_{use_hessian}_batch_size_{batch_size}_distance_{DIST}_steps_{STEPS}_norm_{NORM}_random_{RANDOM}.npy")
