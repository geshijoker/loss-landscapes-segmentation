...
python seg_examples/eval_Fiber_gtrue_crf.py -d /global/cfs/projectdirs/m636/Vis4ML/Fiber/Quarter -r /global/cfs/cdirs/m636/geshi/exp/Fiber/crf/CrossEntropy/2_seed_9999/iter19-10-26-2023-21:51:01.pt -a unet-crf -s 9999 -g 0 -p 1 -ad 5 -aw 32 -ip 288 -bs 128 --benchmark --verbose
"""
...
def test_CRF(model, dataloader, num_classes, device):
    EPS = 1e-7
    since = time.time()
...
cl_wise_iou, test_stats = test_CRF(model, dataloader, n_classes, device)
with SummaryWriter(log_path) as w:
    w.add_hparams({'name':model_name+'_gtrue_crf', 'bs': batch_size}, test_stats)
